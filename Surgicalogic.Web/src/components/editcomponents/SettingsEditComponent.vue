      ...
      <v-card class="container fluid grid-list-md">
        ...
        <v-card-text>
            ...
            <v-flex xs12 sm6 md6  v-show="0">
                <v-autocomplete v-model="selectSettingDataType"
                                :items="settingDataTypes"
                                :label="$t('settings.settingDataType')"
                                :filter="customFilter"
                                @change="settingDataTypeChanged()"
                                disabled="true"
                                item-text="name"
                                item-value="id">
                </v-autocomplete>
              ...
              <v-flex  xs12 sm6 md6 v-show="showInt">
                <v-text-field v-model="editAction['intValue']"
                              :label="$t('settings.value')">
                </v-text-field>
              </v-flex>

               <v-flex xs12 sm6 md6 v-show="showString">
                <v-text-field v-model="editAction['stringValue']"
                              :label="$t('settings.value')">
                </v-text-field>
              </v-flex>

              <v-flex xs12 sm6 md6 v-show="showTime">
                <v-text-field v-model="editAction['timeValue']"
                              :label="$t('settings.value')"
                              :value="editAction['timeValue']"
                              type="time">
                </v-text-field>
              </v-flex>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      vm.$store.dispatch('updateSetting', {
        id: vm.editAction.id,
