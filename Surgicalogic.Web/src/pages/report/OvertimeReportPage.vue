  ...
  <div class="container fluid grid-list-md">
    <div class="grid-card v-card">
      <div class="v-card__text layout row wrap">
        ...
        <v-flex xs12 sm6 md6>
          <v-autocomplete v-model="selectDoctor" :items="doctors" :label="$t('personnel.doctor')" box :filter="customFilter" item-text="personnelTitleName" item-value="id">
          </v-autocomplete>
        </v-flex>
    ...
    <grid-component :headers="headers" :items="overtimeOperations" :title="title" :show-detail="false" :show-edit="false" :show-delete="false" :show-search="true" :show-insert="false" :hide-actions="false" :methodName="getMethodName" :loading="getLoading" :totalCount="getTotalCount" @detail="detail" @exportToExcel="exportOvertimeReportToExcel" @edit="edit" @newaction="addNewItem" @deleteitem="deleteItem">
    </grid-component>
  </div>
</template>
